...
public class ProjectDirectory(string value)
{
    ...
    public bool Contains(string path) => path.StartsWith(value);
    ...
    public Result<string> Join(params string[] paths) =>
        from path in @try(() => string.Join(Path.DirectorySeparatorChar, [value, ..CleanPaths(paths)]))
        from fullPath in @try(() => Path.GetFullPath(path))
        select Contains(fullPath)
            ? ok(fullPath)
            : error("Invalid path");

    public IEnumerable<string> GetDirectories(IFileSystem fileSystem) => fileSystem.GetDirectories(value);

    public override string ToString() => nameof(ProjectDirectory);
